...
static long collatz_steps(unsigned long long n, long *even_count, long *odd_count) {
    long steps = 0;
    while (n != 1) {
        if (n % 2 == 0) {
            n = n / 2;
...
int main(int argc, char *argv[]) {
    long n_values = (argc > 1) ? atol(argv[1]) : 200000;
    unsigned long long max_start = (argc > 2) ? strtoull(argv[2], NULL, 10) : 1000000ULL;
